...
<%--            <h2><asp:TextBox ID="txtThirdBoxtitle" runat="server" TextMode="SingleLine" Width="1024px"></asp:TextBox></h2>--%>
        ...
        <div class="panel-body, center">
            ...
            <br />
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:casecompetitionConnectionString %>" DeleteCommand="DELETE FROM [EventInfo] WHERE [EventID] = @EventID" InsertCommand="INSERT INTO [EventInfo] ([EventName], [EventDate], [EventStartTime], [EventEndTime], [EventLocation], [EventDescription], [EventSpeaker]) VALUES (@EventName, @EventDate, @EventStartTime, @EventEndTime, @EventLocation, @EventDescription, @EventSpeaker)" SelectCommand="SELECT * FROM [EventInfo] ORDER BY [EventDate], [EventStartTime], [EventName]" UpdateCommand="UPDATE [EventInfo] SET [EventName] = @EventName, [EventDate] = @EventDate, [EventStartTime] = @EventStartTime, [EventEndTime] = @EventEndTime, [EventLocation] = @EventLocation, [EventDescription] = @EventDescription, [EventSpeaker] = @EventSpeaker WHERE [EventID] = @EventID">
                <DeleteParameters>
